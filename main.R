...
scenarios <- list(
  list(id = "test", func = gen_scen_test, title = "xxx test xxx")
)
# scenarios <- list(
...
for (n_val in c(N_OBS)) {
  ...
  TARGET_GRID <<- seq(n_val, floor(n_val * 0.70), by = -floor(n_val * 0.10))
  ...
  for (scen in scenarios) {
    ...
    res <- run_simulation_block(scen$func, run_methods = c("PSM", "CEM", "CART"), 
                                cutoff_percentiles=seq(0.1, 0.9, by=0.1)) #
